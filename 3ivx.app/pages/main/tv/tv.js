...
(function () {
    ...
    WinJS.UI.Pages.define("/pages/main/tv/tv.html", {
        ...
        ready: function (element, options) {
            ...
            this._hub = this._layoutRoot.querySelector(".hub-tv").winControl;
            ...
            this._popularData = [];
            this._pagePromises.push(MyApp.Services.Series.getPopularData().then(function (results) {
                this._popularData = results;
            }.bind(this)));
            ...
            MyApp.Utilities.Animation.useCustomHubEntranceAnimation(this._layoutRoot.querySelector(".hub-tv"));
        ...
        unload: function () {
            ...
            document.querySelector(".hub-tv").winControl.removeEventListener("loadingstatechanged", this._handleLoadingStateChangedBind);
        ...
        _handleLoadingStateChanged: function (ev) {
            ...
            setImmediate(function afterPageRenderingHasFinished() {
                var initialFocusElement = this._layoutRoot.querySelector(".layout-tv-section1-slot1");
            ...
            var newTVItems = document.querySelectorAll(".hub-tv .newtv");
            for (var i = 0; i < newTVItems.length; i++) {
                MyApp.Utilities.loadTile(newTVItems[i].winControl, this._newReleaseData[i], "/pages/tvdetails/tvdetails.html");
            }

            document.querySelector(".layout-tv-section1-more").winControl.addEventListener("invoked", function handleClick() {
                WinJS.Navigation.navigate('/pages/browseAll/browseAll.html', { pageTitle: WinJS.Resources.getString("browseNewReleasesPageTitle").value, items: this._newReleaseData });
            }.bind(this));

            // Get all the popular data, assign their MediaTile's metadata properties
            // and create click handlers that navigate to the details page.
            var tvPopularItems = document.querySelectorAll(".hub-tv .tvpopular");
            for (var i = 0; i < tvPopularItems.length; i++) {
                MyApp.Utilities.loadTile(tvPopularItems[i].winControl, this._popularData[i], "/pages/tvdetails/tvdetails.html");
            }

            document.querySelector(".layout-tv-section2-more").winControl.addEventListener("invoked", function handleClick() {
                WinJS.Navigation.navigate('/pages/browseAll/browseAll.html', { pageTitle: WinJS.Resources.getString("browsePopularPageTitle").value, items: this._popularData });
            }.bind(this));

            // Add Search
            this._layoutRoot.querySelector("#tv-search").winControl.addEventListener("invoked", function (evt) {
                utils.Search.handleSearchButtonInvoked(evt, "tv");
            }, false);

            // Add Browse TV
            this._layoutRoot.querySelector("#tv-browse").winControl.addEventListener("invoked", function handleClick() {
                WinJS.Navigation.navigate('/pages/browseAll/browseAll.html', {
                    pageTitle: WinJS.Resources.getString("browsetvPageTitle").value,
                    virtualizedDataFunction: MyApp.Services.Series.getRange,
                    getMaxResultsCountFunction: MyApp.Services.getMaxSeriesDataCount
                });
            });
